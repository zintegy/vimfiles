# Generated by Powerlevel10k configuration wizard on 2025-11-17 at 21:40 UTC.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    worktree_warning        # warning when in wrong worktree
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    # wifi                  # wifi speed
    worktree              # git worktree name (set by use-worktree)
  ...
  # Worktree warning segment - shows warning triangle on the left when in wrong worktree.
  function prompt_worktree_warning() {
    local current_repo=""
    if [[ "$PWD" == "$HOME/ans"* ]]; then
      current_repo="ans"
    elif [[ "$PWD" =~ ^$HOME/([^/]+) ]]; then
      local dir_name="${match[1]}"
      # Only treat as an ans worktree if .git is a file pointing into
      # ans/.git/worktrees/ -- skip standalone repos (e.g. ~/poe2).
      if [[ -f "$HOME/$dir_name/.git" ]] && grep -q "gitdir:.*ans/\.git/worktrees" "$HOME/$dir_name/.git" 2>/dev/null; then
        current_repo="$dir_name"
      fi
    fi

    local activated_repo=""
    if [[ -n "$ANS_WORKTREE" ]]; then
      activated_repo="$ANS_WORKTREE"
    elif [[ "$ANS_ROOT" == "$HOME/ans" ]]; then
      activated_repo="ans"
    fi

    if [[ -n "$current_repo" && -n "$activated_repo" && "$current_repo" != "$activated_repo" ]]; then
      p10k segment -f 3 -t " ⚠⚠⚠ "
    fi
  }

  function instant_prompt_worktree_warning() {
    prompt_worktree_warning
  }

  # Worktree prompt segment - shows current git worktree name when using use-worktree.
  # Shows warning if you're in a different worktree than the one activated.
  function prompt_worktree() {
    # Check if we're in any worktree or ans directory
    local current_repo=""
    if [[ "$PWD" == "$HOME/ans"* ]]; then
      current_repo="ans"
    elif [[ "$PWD" =~ ^$HOME/([^/]+) ]]; then
    ...
    if [[ -n "$current_repo" ]]; then
      if [[ -n "$activated_repo" && "$current_repo" != "$activated_repo" ]]; then
        # Wrong worktree - show warning in yellow
        p10k segment -f 3 -t "⚠ active: $activated_repo"
      elif [[ "$current_repo" == "$activated_repo" ]]; then
        # Correct worktree - show in green
        p10k segment -f 2 -t "$current_repo"
      fi
    fi
  }

  # Instant prompt function for worktree segment.
  function instant_prompt_worktree() {
    prompt_worktree
  }
  ...
  # Custom segment: used RAM in GB
  function prompt_ram_used() {
    local total avail
    total=$(awk '/^MemTotal:/ {print $2}' /proc/meminfo)
    avail=$(awk '/^MemAvailable:/ {print $2}' /proc/meminfo)
    local used_gb=$(( (total - avail) / 1048576.0 ))
    p10k segment -f 6 -t "$(printf '%.1fG' $used_gb)"
  }
